...
schemas_list <- apply(changed_manifests_df, 1, as.list)
output <- list(
  manifest_schemas = schemas_list,
  service_version = changed_manifests_df$service_version[1],
  schema_version = changed_manifests_df$schema_version[1]
)
jsonlite::write_json(output, "changed-manifests.json", pretty = TRUE, auto_unbox = TRUE)
```
