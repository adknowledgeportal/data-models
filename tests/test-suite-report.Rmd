...
rmd_path <- normalizePath(knitr::current_input())
rmd_dir <- dirname(rmd_path)

out_path <- file.path(rmd_dir, "changed-manifests.json")
jsonlite::write_json(output, out_path, pretty = TRUE, auto_unbox = TRUE)

if (!file.exists(out_path)) stop("JSON file was not written to the given path.")
print(getwd())
```
